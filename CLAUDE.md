...
| 模块 | 职责 |
|---|---|
| `jforge-annotation` | 注解：`@Table/@Id/@Column/@GeneratedValue`（标注接口方法）+ `@Dao/@Query/@Bind/@ReturnGeneratedKeys` + `@Select/@Where/Op`（声明式查询）+ JSpecify 依赖（`@Nullable` 动态条件：@Query 方括号 `[ ]` 显式段 + 自动推断，@Select 参数条件） |
