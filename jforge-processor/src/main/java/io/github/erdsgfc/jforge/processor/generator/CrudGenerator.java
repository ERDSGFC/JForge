...
public final class CrudGenerator {
    ...
    private MethodSpec saveMethod(JForgeProcessor.DaoInfo info, ClassName entityImpl, ClassName connection,
        ...
        if (model.idGenerated()) {
            ...
            appendIdWriteback(method, model, entityImpl, "entity");
    ...
    private static void appendInBindings(MethodSpec.Builder method, JForgeProcessor.DaoInfo info) {
        ...
        method.addCode(idBindParam(info));
        method.addCode("\n");
    ...
    private void appendBatchKeysWriteback(MethodSpec.Builder method, JForgeProcessor.DaoInfo info,
        ...
        method.beginControlFlow("while (rs.next() && i < $L)", "batchStart");
        appendIdWriteback(method, info.model, entityImpl, "entities.get(i)");
        method.addStatement("i++");
        method.endControlFlow();
        method.endControlFlow();
    }
    ...
    public MethodSpec joinRowMapperMethod(JForgeProcessor.DaoInfo info, ClassName entityImpl, ClassName sqlException, ClassName resultSet) {
        ...
        for (int i = 0; i < columns.size(); i++) {
            EntityModel.ColumnModel column = columns.get(i);
            method.addCode(SqlCodegen.readColumn(column.typeName, column.javaType, column.javaClassType, "e", column.setterName, "startIndex + " + i,
